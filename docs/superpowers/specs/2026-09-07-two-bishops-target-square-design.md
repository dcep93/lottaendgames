...
King-wall move restriction (2026-09-08): add a visible `king wall` policy guard after mate, bishop safety, and no stalemate, before r3 and all positional preferences. A king move ending on either member of an adjacent parallel bishop-diagonal pair is rejected by this guard unless the destination is exactly one square from its nearest board edge. Check both diagonal axes using the starting bishop positions, including short finishing walls; no enclosure-size floor or alternate wall-profile choice can bypass this movement guard. Board-edge destinations have distance zero and do not satisfy the exception. Moves off all walls and bishop moves are neutral, including bishop moves while the stationary king already occupies a wall. Keep actual chess legality, existing ray-screen counting, and the no-farther target qualification unchanged. Higher universal priorities retain precedence.

In `5B2/3k4/8/8/4K3/1B6/8/8 w - - 18 10`, Kd5 lands centrally on outer wall a2–g8 and screens Bb3. Ke5 lies outside both walls on a1–h8, so the guard rejects Kd5 and the policy selects Ke5. This restriction supersedes earlier recorded positional preferences that require forbidden wall steps. Preserve direct tests of those lower rules and separately assert the guard's overriding selection. Verify inner/outer walls, all eight symmetries, exact edge distances zero/one/two-or-more, both axes, short walls, moves off walls, stationary-king bishop moves, guide placement, and the old mating witnesses. Record new counterexamples without changing unrelated rules.
