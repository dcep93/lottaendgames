...
Keep the preference in r15 and preserve its three subsequent tie-breaks. The user resolved the priority question with a follow-up r10 eligibility condition: precage proximity applies only when the knight is behind White's king. This keeps the rule order intact and allows immediate Ne2. See the r10-behind design.
