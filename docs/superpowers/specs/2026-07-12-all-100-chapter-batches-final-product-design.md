# All 100 Endgames Final Product Design

## Goal

Turn Lotta Endgames from a chapter sample into a complete, snappy reader for the 100 endgames. The job is not complete at "all represented chapters are present"; it must also finalize the app with a strict perfect pass that drives the SAN audit to zero unresolved misses. The final app should feel consistent across the endgame chapters, render chapter switches quickly, keep all reconstructable played and variation moves clickable, and make any non-clickable prose an explicit audited design choice.

## Product Bar

The product-complete milestone is Endings 1-100 available in the app from the single async, content-hashed chapter payload, with zero unresolved SAN audit misses across the included endgame content. This is focused on the 100-endgames body of the book, not differently styled front matter or other non-endgame chapters. Those can be handled later.

Book chapters 3 and 4 are currently missing and must be added because they contain the early endgames before Chapter 5 starts at Ending 21. Chapters 5-13 are already represented and cover the remainder through Ending 100, though they still need cleanup and final audit work. Each endgame chapter should render through the same components and data flow, regardless of whether it came from the early hand-built extraction or a later batch pipeline.
...
Process the remaining 100-endgames body in committed batches:

- checkpoint the current verified chapter 10-12 cleanup first;
- add and verify Chapter 3, covering its early endgame range;
- add and verify Chapter 4, covering the remaining pre-Chapter-5 endgames;
- clean up and verify the already represented Chapters 5-13 as needed;
- run the strict finalization pass across all included endgame chapters.

If Chapters 3 or 4 are too large for one clean pass, split them by ending ranges, not by invented chapter numbers. Each batch should be shippable on its own. A batch includes extraction, normalization, safe diagram promotion, payload regeneration, tests, and a commit. JSON should remain generated from the pipeline; do not hand-patch chapter JSON when a source extraction or normalization rule is the real fix.

After Chapter 3 and Chapter 4 are represented, continue directly into finalization rather than stopping. Finalization includes clearing the strict audit to zero unresolved misses, removing the current page-banner copy, fixing chapter-switch performance, and verifying the full app acceptance criteria.
...
The viewer should avoid recomputing expensive chapter playback for chapters that are not being rendered. Derived data such as parsed move tokens, navigation trees, and playable-position sets should be memoized by chapter identity and payload hash. Chapters 3-4, 5-9, and 10-13 should use the same rendering path; if a chapter requires fallback behavior, that fallback should be explicit and cheap.
...
- a table of contents or endgame index once all endgame chapters exist;
...
Search is a good final polish feature after all endgame chapters load reliably, but it should not block content completion.

## Finalization Pass

Once Endings 1-100 are included, run a dedicated finalization pass that finishes the app as a product. This pass must:

- make the strict audit cover all included endgame chapters;
...
- confirm chapters 3-13 use the same rendering path;
...
- Endings 1-100 are present in the single content-hashed async payload;
- book chapters 3 and 4 are represented, while non-endgame/front-matter chapters can remain out of scope;
...
- the strict audit has zero unresolved misses across all included endgame chapters;
