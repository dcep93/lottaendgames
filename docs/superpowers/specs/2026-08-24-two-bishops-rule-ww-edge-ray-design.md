...
Before White moves, discard a ray when a bishop already controls its first square. The second square of every remaining on-board ray is a Rule WW target. The rule is inactive when there are no such targets, including when Black is not on an edge or a ray does not contain two further board squares.

## Move ranking

A candidate White move receives the preferred score when, in the resulting position, either bishop controls at least one Rule WW target through an unobstructed line and that target is not adjacent to White's resulting king. The bishop need not have moved and may preserve control it already supplied. Qualifying targets tie; Rule WW does not maximize the number of targets controlled. Later rules break remaining ties.
...
- Reject a controlled second-square target adjacent to White's resulting king. In the h1 fixture, `Be2` loses because `f1` is beside the king on `f2`, and `Kg3` loses because `h3` becomes adjacent to the resulting king on `g3`.
