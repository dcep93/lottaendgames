...
- White king: `e5`.
- Black king: `f7`, offset `(+1,+2)` from White's king.
- White bishops: `e4` and `f4`, offset `(0,-1)` and `(+1,-1)`.
- Required move: `Kd6`, offset `(-1,+1)` from White's king.

This geometry replaces the earlier `f5/g7` draft; that arrangement must not
activate `reverse conclave step`.
...
an arrow from `e5` to `d6`. Add it to the guide immediately after the existing
...
- `Kd6` is selected and explained by `reverse conclave step` in the canonical
  position;
- all D4 transforms select the transformed king step;
- at least one translated arrangement selects its translated king step;
- the earlier draft, nearby arrangements, and Phase 2 positions do not
  activate the rule;
