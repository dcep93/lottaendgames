...
test('priority guide follows registered facade order and renders typed diagrams', () => {
  ...
  assert.doesNotMatch(decodedMarkup, /rule (?:r3\.8|r4\.5|r15|r18|r20|s10)\b|king closer|mating net/)
  assert.ok(decodedMarkup.includes("rule r9.1</strong> — If a bishop is attacked, maximize its distance from Black's king, unless it's defended by White's central king."))
  ...
  assert.ok(decodedMarkup.includes("rule r9.2</strong> — If a knight is attacked, defend it with the king or otherwise maximize its distance from Black's king, then minimize its distance to White's king."))
  assert.ok(decodedMarkup.includes("rule r9.3</strong> — If a bishop and knight are both within 2 steps of Black's king, unless they're defended by a central king, maximize the Bishop's distance from Black's king."))
  ...
  assert.match(markup, /rule-r4-flush\.gif/)
  assert.ok(!decodedMarkup.includes('rule r6</strong>'))
  assert.doesNotMatch(markup, /solidify-seven-diagonal\.gif/)
  assert.doesNotMatch(markup, /central-seven-diagonal\.gif/)
  const animation = [...markup.matchAll(/<figure\b[^]*?<\/figure>/g)].map(match => match[0]).find(figure => figure.includes('rule-r4-flush.gif'))
  assert.ok(animation)
...
test('Two Bishops keeps only the target-square note and all diagrams', () => {
...
test('Two Bishops shows r10 outer-bishop conditions and its changed exceptions', () => {
  ...
  assert.equal(ruleSet.help.noteBoards.some(({ id }) => id === 'two-bishops-rule-r10-edge'), false)
})
...
test('Two Bishops explains the r19 outer-diagonal proximity condition', () => {
  ...
  assert.match(markup, /Euclidean proximity to that same Phase 2 king square/)
})
