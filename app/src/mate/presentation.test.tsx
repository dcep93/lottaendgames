...
test('priority guide follows registered facade order and renders typed diagrams', () => {
  ...
  assert.ok(decodedMarkup.includes("Prefer king Euclidean proximity to the center, then king off bishop's color, then bishop on the long diagonal, then a protected central bishop, then knight move proximity to a precage square only when the knight is behind White's king from Black's king's perspective."))
  ...
  assert.ok(decodedMarkup.includes("rule r9.5</strong> — Prefer to not have an attacked knight defended by a bishop."))
