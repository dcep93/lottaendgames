...
test('r6.8 rejects the loaded double attack before centralization across D4', () => {
  ...
  for (const t of SQUARE_TRANSFORMS) {
    ...
    assert.deepEqual(getIdealKnightAndBishopWhiteMoves(fen), [move('c5', 'd5')])
  }
})

test('r6.8 allows a double attack when either minor piece is defended', () => {
  const cases = [
...
test('r6.8 ignores a geometric fork on an illegal Black destination', () => {
