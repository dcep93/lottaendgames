...
test('r10 escapes the loaded nearby bishop with a noncentral king in every symmetry', () => {
  for (const transform of SQUARE_TRANSFORMS) {
    ...
    assert.equal(scoreKnightAndBishopWhiteMove(fen, move('h1')).nonCentralKingBishopEscapeScore, -Math.sqrt(61));
    assert.equal(scoreKnightAndBishopWhiteMove(fen, move('a8')).nonCentralKingBishopEscapeScore, -Math.sqrt(5));
    assert.deepEqual(getIdealKnightAndBishopWhiteMoves(fen), [move('h1')], transform.name);
  }
});

test('r10 checks the two-step range and central king before White moves', () => {
  for (const transform of SQUARE_TRANSFORMS) {
    for (const [fen, from, to, expected] of [
      ...
      // Starting centrally exempts the rule, even if the king leaves the center.
      ['8/8/5k2/8/3KB3/8/N7/8 w - - 0 1', 'd4', 'c3', 0],
      ...
      assert.equal(scoreKnightAndBishopWhiteMove(source, san).nonCentralKingBishopEscapeScore, expected);
    }
  }
});
