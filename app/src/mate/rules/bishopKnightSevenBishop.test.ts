...
test('a3 flush walks toward b2 on the opposite color only while retaining seven support', () => {
  for (const transform of SQUARE_TRANSFORMS) {
    for (const start of [
      ...
      assert.deepEqual(getIdealKnightAndBishopWhiteMoves(reflected),[san('d6')])
      const d6 = scoreKnightAndBishopWhiteMove(reflected,san('d6'))
...
test('a-file Bd7 support restores the existing Kc5 preference', () => {
...
test('supported five with Nd5 and Black near a5 approaches b4, including reflections', () => {
  for (const transform of SQUARE_TRANSFORMS) {
    const position = transformFen('8/3B4/k7/2KN4/8/8/8/8 w - - 0 1', transform)
    ...
    // Bishop placement now precedes this king target; compare supported king moves in isolation.
    const kingCandidates = ['b4', 'd6'].map(to => ({san: san(to as 'b4' | 'c4' | 'd6'), score: scoreKnightAndBishopWhiteMove(position, san(to as 'b4' | 'c4' | 'd6'))}))
...
test('supported five bishop and five knight prefer b5 or d7 equally before king targets', () => {
  for (const transform of SQUARE_TRANSFORMS) {
    ...
    for (const to of ['b5', 'd7', 'c6'] as const) {
      ...
      assert.equal(score.supportedDiagonalSizeScore, to === 'c6' ? 99 : 5)
      assert.equal(score.supportedFiveBishopPenalty, 0)
    }
    assert.equal(scoreKnightAndBishopWhiteMove(position, move('d6', 'e7')).supportedFiveBishopPenalty, 1)
    // This is the a8–h1 reflection of the declared Kb4 after Be8 Ka6.
    assert.deepEqual(getIdealKnightAndBishopWhiteMoves(position), [move('d6', 'e7')])
    // Without the declared king move, the two bishop destinations still tie.
    const bishopCandidates = ['b5', 'd7'].map(to => ({san: move('a4', to as 'b5' | 'd7' | 'c6'), score: scoreKnightAndBishopWhiteMove(position, move('a4', to as 'b5' | 'd7' | 'c6'))}))
...
test('r2.5 prescribes Be8 then Kb4 in the supported five loop, including D4 reflections', () => {
  for (const transform of SQUARE_TRANSFORMS) {
    const chess = getChess(transformFen('8/3B4/8/k1KN4/8/8/8/8 w - - 41 23', transform))
    for (const [from, to, replyFrom, replyTo] of [['d7', 'e8', 'a5', 'a6'], ['c5', 'b4', 'a6', 'b7']] as const) {
      const position = chess.fen()
      const move = {from: transformSquare(from, transform), to: transformSquare(to, transform)}
      const san = getChess(position).move(move).san
      const score = scoreKnightAndBishopWhiteMove(position, san)
      assert.equal(score.supportedDiagonalSizeScore, 5)
      assert.equal(score.declaredSupportedFivePenalty, 0)
      assert.deepEqual(getIdealKnightAndBishopWhiteMoves(position), [san])
      assert.equal(getMateRuleSet('bishop-knight').currentWhiteHint(position)?.id, 'r2.5')
      const candidates = bishopKnightRuleSet.scoreWhiteCandidates!(position, bishopKnightRuleSet.whiteMoves(position))
      const earlier = knightAndBishopWhiteRules.slice(0, knightAndBishopWhiteRules.findIndex(r => r.id === 'r2.5'))
...
test('r2.5 prescribes loaded second-move Kd8 from Ke7 Bb5 Nd5 against Kb7', () => {
  for (const transform of SQUARE_TRANSFORMS) {
    for (const counters of ['2 2', '41 23']) {
      const fen = transformFen(`8/1k2K3/8/1B1N4/8/8/8/8 w - - ${counters}`, transform)
      const san = getChess(fen).move({from: transformSquare('e7', transform), to: transformSquare('d8', transform)}).san
      const score = scoreKnightAndBishopWhiteMove(fen, san)
      assert.equal(score.supportedDiagonalSizeScore, 5)
      assert.equal(score.declaredSupportedFivePenalty, 0)
      assert.deepEqual(getIdealKnightAndBishopWhiteMoves(fen), [san])
    ...
    const nearby = transformFen('1k6/4K3/8/1B1N4/8/8/8/8 w - - 0 1', transform)
    for (const san of getChess(nearby).moves()) assert.equal(scoreKnightAndBishopWhiteMove(nearby, san).declaredSupportedFivePenalty, undefined)
  }
})
