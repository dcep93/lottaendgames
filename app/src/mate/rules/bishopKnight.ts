...
function whiteScoringContext(fen: string): KnightAndBishopPositionScoreContext {
  ...
  return {
    ...
    startsWithNearbyKings: !!whiteKing && !!blackKing && kingDistance(whiteKing.square, blackKing.square) <= 2,
...
function scoreKnightAndBishopWhiteMoveCore(
  ...
  return {
    ...
    get knightBishopProtectionPenalty() {
      return bishop && knight && kingDistance(bishop.square, knight.square) >= 2
        && bishopControlsOrOccupiesSquare(resultFen, bishop.square, knight.square, blackKing?.square) ? 0 : 1;
    },
    ...
    kingBishopColorPenalty: whiteKing && bishop && squareColor(whiteKing.square) === squareColor(bishop.square)
      && !(move.piece === "k" && CORNERS.includes(move.from)) ? 1 : 0,
