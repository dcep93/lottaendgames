...
test('r6 drifts from distant bishop defense in the loaded Ba4 Nd1 position across D4',()=>{
 for(const t of SQUARE_TRANSFORMS){
  ...
  assert.equal(keep.knightStableBishopProtectionPenalty,1,t.name);
  assert.equal(alsoKeep.knightStableBishopProtectionPenalty,1,t.name);
  ...
  assert.ok(r6.compare!(keep,alsoKeep)<0,t.name);
  assert.deepEqual(getIdealKnightAndBishopWhiteMoves(fen),[knight],t.name);
 }
});

test('r6 rejects distant bishop defense regardless of bishop edge status across D4',()=>{
 const cases=[
  {fen:'8/8/2B5/1N6/8/8/5k2/7K w - - 0 1',penalty:1},
