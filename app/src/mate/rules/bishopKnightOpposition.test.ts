...
test('opposition metric remains neutral to the r7 move choice across D4', () => {
  for (const t of SQUARE_TRANSFORMS) {
    ...
    assert.ok(scoreKnightAndBishopWhiteMove(fen, san).bishopOppositionPenalty < scoreKnightAndBishopWhiteMove(fen, central).bishopOppositionPenalty)
    assert.ok(!getIdealKnightAndBishopWhiteMoves(fen).includes(san))
  }
})

test('opposition metric can interpose the bishop or maintain existing opposition with a knight move', () => {
  for (const t of SQUARE_TRANSFORMS) {
...
test('opposition metric requires the bishop in the middle of direct rank or file opposition', () => {
