...
test('r6 has no special preference for establishing a precage knight across D4', () => {
 ...
 for (const t of SQUARE_TRANSFORMS) {
  ...
  // Keep the existing stable Bg8–Nc4 defense while the king approaches.
...
test('r6 ties stable bishop defenses regardless of their king-protection distance', () => {
...
test('r6 prefers stable Nb5 defense over the viable Ne2 drift after Nd4 Ke5 across D4', () => {
 const rule = knightAndBishopWhiteRules.find(r => r.id === 'r6')!;
