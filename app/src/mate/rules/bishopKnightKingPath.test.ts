...
test('r9.95 keeps the bishop between the kings before r10 can force its escape', () => {
  for (const t of SQUARE_TRANSFORMS) {
    ...
    // Without r6, the king can improve centrality while the bishop stays between the kings.
    const ke6 = getChess(fen).move({from: transformSquare('f6', t), to: transformSquare('e6', t)}).san;
    assert.deepEqual(getIdealKnightAndBishopWhiteMoves(fen), [ke6], t.name);
  }
});
