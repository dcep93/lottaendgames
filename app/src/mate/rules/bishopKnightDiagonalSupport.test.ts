...
test('White on a lower target-corner diagonal disqualifies support across D4', () => {
  for (const transform of SQUARE_TRANSFORMS) {
    ...
    // Both kings are on diagonal four, even though White is closer by king steps.
    assert.equal(knightAndBishopSupportedDiagonal(transformFen(
      '6B1/2K5/8/k7/8/3N4/8/8 b - - 0 1', transform)).size, 7)
    const bf7Before = transformFen('8/8/8/8/3K4/k2N4/B7/8 w - - 2 2', transform)
    const bf7Board = getChess(bf7Before)
    const bf7 = bf7Board.move({from: transformSquare('a2', transform), to: transformSquare('f7', transform)}).san
    // Kd4 is on diagonal eight; Ka3 is on six, so Bf7 keeps support.
    assert.equal(knightAndBishopSupportedDiagonal(bf7Board.fen()).size, 7)
    assert.equal(scoreKnightAndBishopWhiteMove(bf7Before, bf7).supportedDiagonalSizeScore, 7)
  }
})
...
test('declared Bb1 rejection and nearby king-color restriction apply across D4', () => {
  for (const transform of SQUARE_TRANSFORMS) {
    ...
    for (const [nearby, expected] of [
      ...
      ['8/8/8/8/2N5/4K3/8/1B1k4 b - - 1 1', 7] // White's diagonal is beyond Black's.,
...
test('Bd7 permits equal king diagonals while Kd6 retains support', () => {
  for (const transform of SQUARE_TRANSFORMS) {
