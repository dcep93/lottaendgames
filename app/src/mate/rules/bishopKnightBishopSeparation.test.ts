...
test('r6.6 separates the bishop from a king outside the middle 16, across D4',()=>{
 for(const t of SQUARE_TRANSFORMS){
  ...
  const rule=knightAndBishopWhiteRules.find(r=>r.id==='r6.6')!;
  assert.ok(rule.compare!(far,threeSteps)<0,t.name);
  assert.ok(rule.compare!(threeSteps,twoSteps)<0,t.name);
  const check=getChess(fen).move({from:transformSquare('c5',t),to:transformSquare('a6',t)}).san;
  assert.deepEqual(getIdealKnightAndBishopWhiteMoves(fen),[check],t.name); // Earlier r5 remains authoritative.
  assert.equal(explainMove(bishopKnightRuleSet.scoreWhiteCandidates!(fen,getChess(fen).moves()),knightAndBishopWhiteRules,check)?.id,'r5',t.name);
  ...
  assert.equal(explainMove(bishopKnightRuleSet.scoreWhiteCandidates!(active,getChess(active).moves()),knightAndBishopWhiteRules,retreat)?.id,'r6.6',t.name);
...
test('r6.6 uses the starting king and bishop arrangement, across D4',()=>{
 for(const t of SQUARE_TRANSFORMS){
  for(const fen of [
