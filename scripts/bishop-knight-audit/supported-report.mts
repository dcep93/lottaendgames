export function supportedReport(r: any, manifest: any): string {
    ...
    let text = `# ${r.population === 'all' ? 'Full-position' : 'Supported-position'} continuation audit\n\nPolicy commit: \`${manifest.commit}\`. Fingerprint: \`${manifest.fingerprint}\`.\n\nThe full placement census classified ${num(c.legal)} positions and selected **${num(total)} ${stage} post-White placements** as starts. Continue through supported and unsupported positions; stop only at mate, capture, or stalemate. All tied preferred White moves and every legal Black reply are included; return history is irrelevant.\n\n| Selected starts | Positions | Percentage |\n|---|---:|---:|\n`;
    for (const [label, n] of rows) text += `| ${label} | ${num(n)} | ${pct(n)} |\n`;
    text += `\n${num(r.graph.cyclicFamilies)} cyclic components; ${num(r.graph.nodes)} board states and ${num(r.graph.edges)} transitions. Terminal outcomes can overlap across tied branches. Direct membership counts selected post-White boards on cycles; a cycle may also contain unsupported boards. Zero loops does not imply forced mate. This is a placement census, not retrograde reachability proof. White follows app preferences; Black can play any legal reply. Clocks and repetition claims are excluded.\n\n## Representative loops\n\n| Component | Selected starts reaching it | Replay |\n|---|---:|---|\n`;
    for (const f of [...r.families].sort((a: any, b: any) => b.reachablePlacements - a.reachablePlacements))
        text += `| ${f.id} | ${num(f.reachablePlacements)} | [${f.witness.moves.join(' ')}](${f.witness.url})${f.witness.freshLoadVerified ? '' : ' (replay validation failed)'} |\n`;
    if (r.placements) {
