...
test('example verifier distinguishes eliminated drift loops from a surviving protected-knight shuffle', async () => {
  const {verifyLoopExample} = await import('./verify-loop-example.mts');
  ...
  assert.equal(verifyLoopExample("2BK4/8/1k6/N7/8/8/8/8 w - - 0 1", ["Nb7", "Kc6", "Na5+", "Kb6"]), false);
});
