...
if(args.includes('--help')) {
 console.log('npm run audit:unsupported -- --out /absolute/output [--workers 8] [--compare /previous/result.json] [--roots-from /previous/audit] [--scope unsupported|supported|all] [--diagonal 7|5|3] [--gate loops|mate]\nResumes identical policy checkpoints; rejects stale ones. Node >=22.13 required.');process.exit(0);
}
const options={};
...
if(!['unsupported','supported','all'].includes(scope))throw new Error('scope must be unsupported or supported or all');
...
if(scope!=='unsupported'&&options['roots-from'])throw new Error('Supported and full audits require a fresh root census');
...
if(!existsSync(manifestPath))writeFileSync(manifestPath,JSON.stringify({blackPolicy:"all-legal",fingerprint:hash,referenceWorkerFingerprint:referenceHash,rootFingerprint:rootHash,rootCacheSource:rootsFrom||null,commit,startedAt:new Date().toISOString(),node:process.version,workers,population:scope,diagonal:diagonal||null,scope:scope==='all'?'All post-White KBNvK placements; continue through support changes; all preferred White ties and every legal Black reply; no return-history filtering.':scope==='supported'?`All supported ${diagonal ? diagonal+'-diagonal ' : ''}post-White KBNvK placements; continue through support changes; stop at mate/capture/stalemate; all preferred White ties and every legal Black reply; no return-history filtering.`:'All 13,660,584 post-White KBNvK placements; stop at support/mate/capture/stalemate; all preferred White ties and every legal Black reply; no return-history filtering.'},null,2));
...
for(const stage of ['validate','validate-symmetry',...(rootsFrom?['reuse-roots']:[]),'census','analyze','classify','positions','report',...(scope==='all'?['full-details','full-report']:[]),...(options.gate?['gate']:[])]) {
...
console.log('Report:',join(dir,scope==='all'?'full-report.md':'report.md'));
