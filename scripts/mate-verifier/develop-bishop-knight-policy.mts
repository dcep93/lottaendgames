  ...
  for (const root of roots) {
    ...
    if (result === undefined) {
      result = verifyMateRoots([root], adapter, {
        ...
        onProgress: (stats) => {
          if (stats.uniquePositions % 5_000 === 0) {
            console.error(
              `${root.source}: ${stats.uniquePositions} positions`,
            )
          }
        },
        progressEvery: 5_000,
...
const reasonId =
  failure === undefined
    ? undefined
    : getMateRuleSet('bishop-knight').currentWhiteHint(
        failure.startingFen,
      )?.id
const localhostUrl =
  failure === undefined
    ? undefined
    : `http://localhost:5173/mate/bishop-knight#${new URLSearchParams({
        cursor: '0',
...
function developmentGateRoots(): readonly MateVerificationRoot<ProductionMateVerificationState>[] {
  ...
  const randomRoots = sampleBishopKnightRoots(
  ...
  const add = (root: MateVerificationRoot<string>): void => {
    const key = canonicalVerifierPositionKey('bishop-knight', root.state)
  ...
  for (const [index, fen] of smokeFens.entries()) {
    ...
    if (!validateMatePosition('bishop-knight', canonicalFen).ok) {
      throw new Error(
        `Invalid Bishop + Knight smoke root ${index + 1}: ${fen}`,
      )
    }
    add({
      fen: canonicalFen,
...
function parseSmokeRoots(): readonly string[] {
  ...
  if (
    ...
    throw new Error('Bishop + Knight smoke roots must be a JSON string array')
  }
  return parsed
}
